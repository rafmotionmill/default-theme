...
<?php while ( have_posts() ) : the_post() ?>
			...
			<article id="post-<?php the_ID() ?>" class="<?php sandbox_post_class() ?>">
				<header>
					...
					<div class="entry-date"><time class="published" datetime="<?php the_time('Y-m-d') ?>"><?php unset($previousday); printf( __( '%1$s &#8211; %2$s', 'sandbox' ), the_date( '', '', '', false ), get_the_time() ) ?></time></div>
				</header>
